...
import numpy as np
#import time
from fastdtw_cython cimport fastdtw
cimport numpy as np
cimport cython
...
cdef medoid(np.ndarray[DTYPE_FLOAT_t, ndim=2] D):#Find mediod of database
    #t1=time.clock()
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cdef s_contrib(np.ndarray[DTYPE_FLOAT_t, ndim=1] center,np.ndarray[DTYPE_FLOAT_t, ndim=1] s):#Contribution of sequence s to barycenter center
    #t1=time.clock()
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cdef FDBA_update(np.ndarray[DTYPE_FLOAT_t, ndim=2] center_matrix):#Update barycenter according to the center_matrix
    return np.divide(center_matrix[:,0],center_matrix[:,1],dtype=DTYPE_FLOAT)

@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cpdef FDBA(np.ndarray[DTYPE_FLOAT_t, ndim=2] D,int n_iterations=10,bint verbose=False):#Fast DTW barycenter averaging full algorithm
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cpdef assign(np.ndarray[DTYPE_FLOAT_t, ndim=2] centers,np.ndarray[DTYPE_FLOAT_t, ndim=2] D):#assign time series to centers
...
def FDBA_clustering(np.ndarray[DTYPE_FLOAT_t, ndim=2] D,int n_iterations=10,int k=2,bint verbose=False):#kmeans using FDBA
    ...
    for i in range(n_iterations):
        ...
        for j in range(len(centers)):
            centers[j]=FDBA(D[matched_labels==j],10,verbose=verbose)
