...
'''#my code minifunc. it is used in numba version
    def min_ind(a):
    return a[0]'''
#@profile
def __dtw(x, y, window, dist):
    ...
    '''#my code
    D=np.full((window[-1][0]+1,window[-1][1]+1,3),np.inf,dtype=np.float)
    D[0, 0] = np.array([0, 0, 0])
    for i, j in window:
        dt = dist(x[i-1], y[j-1])
        D[i, j] = min((D[i-1, j][0]+dt, i-1, j), (D[i, j-1][0]+dt, i, j-1),
                      (D[i-1, j-1][0]+dt, i-1, j-1), key=min_ind)'''
    ...
    for i, j in window:
        ...
        priors=np.array([(D[i-1, j][0], i-1, j), (D[i, j-1][0], i, j-1),
                      (D[i-1, j-1][0], i-1, j-1)])
        D[i, j] = priors[np.argmin(priors[:,0])]+dt'''
    path = []
    i, j = len_x, len_y
...
if __name__=="__main__":
    main()
    #timeit.timeit("main()","gc.enable; from __main__ import main",number=100)
