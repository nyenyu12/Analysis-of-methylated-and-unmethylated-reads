...
def main():
    #centers = FDBA_cython.FDBA_clustering(X_train,n_iterations=6,k=2,verbose=False)
    #Print profiling statistics using the `line_profiler` API
    profile = line_profiler.LineProfiler(FDBA_cython.s_contrib)
    profile.runcall(FDBA_cython.FDBA_clustering,X_train,n_iterations=6,k=2,verbose=False)
    profile.print_stats()
    #y_pred=FDBA_cython.assign(centers,X_train)
